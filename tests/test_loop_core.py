...
def test_run_loop_core_cycle_promotes_historical_rejection_to_higher_boundary(tmp_path: Path) -> None:
    ...
    queue["rejected"] = [
        {
            "queue_id": "milestone-loop-cycle-199",
            "cycle_id": "loop-cycle-199",
            "candidate_id": "candidate-loop-cycle-199",
            "summary": "rejected governance candidate",
            "classification": "control_plane_policy",
            "target_surface": ["docs/plans/"],
            "promotion_decision": "milestone_promotion_required",
    ...
    report = json.loads((repo_root / Path(proc.stdout.strip())).read_text(encoding="utf-8"))
    assert report["candidate"]["classification"] == "product_contract_rule"
    assert report["promotion"]["decision"] == "user_decision_required"
    assert report["candidate"]["summary"] == "raise the boundary for recurring rejected governance changes"


def test_run_loop_core_cycle_absorbs_stable_governance_pattern_into_internal_rule(tmp_path: Path) -> None:
    repo_root = tmp_path / "repo"
    repo_root.mkdir()
    (repo_root / ".git").mkdir()
    ...
    queue["applied"] = [
        {
            "queue_id": "milestone-loop-cycle-201",
            "cycle_id": "loop-cycle-201",
            "candidate_id": "candidate-loop-cycle-201",
            "summary": "accepted governance candidate one",
            "classification": "control_plane_policy",
            "target_surface": ["docs/plans/"],
            "promotion_decision": "milestone_promotion_required",
        ...
        {
            "queue_id": "milestone-loop-cycle-202",
            "cycle_id": "loop-cycle-202",
            "candidate_id": "candidate-loop-cycle-202",
            "summary": "accepted governance candidate two",
            "classification": "control_plane_policy",
            "target_surface": ["docs/plans/"],
            "promotion_decision": "milestone_promotion_required",
    ...
    assert report["candidate"]["summary"] == "absorb stable governance pattern into internal operating rule"
